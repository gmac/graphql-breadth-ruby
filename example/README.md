...
- `/stream` for `@stream` over SSE. The resolver loads the current cards, then fetches their distinct sets on demand. The first two sets are included in the initial result and the rest are delivered incrementally.
...
curl -N http://localhost:9292/graphql \
  -H 'Accept: text/event-stream' \
  -H 'Content-Type: application/json' \
  -d '{"query":"{ magicSets @stream(initialCount: 2) { code name } }"}'
```
